function spm_reslice_yli(P,flags)
...
if flags.mask || flags.mean
    ...
    % export mask of realignment %yli-20180314
    brnmsk_realign = (Count == numel(P));
    VO         = P(1);
    VO.fname   = spm_file(P(1).fname, 'prefix',flags.prefix, 'suffix','_brnmsk_realign');
    VO.dim     = P(1).dim(1:3);
    VO.dt      = [4 0];
    VO.pinfo   = [1;0;0];
    VO.mat     = P(1).mat;
    spm_write_vol(VO,brnmsk_realign);
