...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('../models','DWAN_native_space_huber_loss_pad_ms_mean10'), type=str, help='directory of the model')
    parser.add_argument('--model_name', default='model_068.hdf5', type=str, help='the model name')
...
if __name__ == '__main__':    
    ...
    def predict_nii_w_masks_rbk(model,root_folder,save_name):
        ...
        for i in range(len(content)):
            ...
            item_path = glob.glob('{}/{}/ASL/cbf_0_s*.nii'.format(root_folder,content[i]))[0]
            ...
            #for Relu updating.
            item[mask[:,:,:] < 0.1] = 2
            y = np.clip(np.mean(item[:,:,:,:],axis=3),0,150)/255.0           
            y_ = np.transpose(y,(2,0,1))            
            y_ = y.reshape((y.shape[0],y.shape[1],y.shape[2],1))
            x_ = model.predict(y_)
            #For ADNI data 150 of cbf is enough
            x_ = np.clip(np.squeeze(x_*255.0),0,150)                                    
            mask[mask>0] = 1    
            x_= x_ * mask
            x_nii = nib.Nifti1Image(x_, item_obj.affine, item_obj.header)            
            save_path = '{}/{}/ASL/{}.nii'.format(root_folder,content[i],save_name)
            nib.save(x_nii,save_path)

    save_name = 'DFT_ADNI_to_Yiran'
    predict_nii_w_masks_rbk(model,'../data/',save_name)              
