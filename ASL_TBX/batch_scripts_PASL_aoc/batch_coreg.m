...
for s = 1:length(PAR.subjects) % for each subject
    ...
    for c=1:PAR.ncond
       ...
       % Ptmp=spm_get('files', PAR.condirs{s,c}, ['r*img']);
        Ptmp=spm_select('EXTFPList', char(PAR.condirs{s,c}), ['^r' PAR.confilters{c} '.*nii'], 1:1000);

        PO=strvcat(PO,Ptmp);
        
       Ptmp=spm_select('EXTFPList', char(PAR.M0dirs{sb,c}),  ['^r' PAR.M0filters{c} '.*nii'], 1:1000);
